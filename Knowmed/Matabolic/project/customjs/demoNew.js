...
$(document).ready(function () {
  ...
  var button = document.getElementById("btnZoomIn"),
        count = 0;
...
function init(data, data1, isInit) {
    if (isInit == 0) {
        var count = 1;
        ...
        myDiagram.nodeTemplate =
            $(go.Node, "Auto",
                ...
                $(go.Shape, "Rectangle",
                    ...
                    new go.Binding("font", "100pt"),
                    //new go.Binding("fill", "highlight", function (v) { return v ? "pink" : "color"; }),
                    new go.Binding("stroke", "highlight", function (v) { return v ? "#39FF14" : "blue"; }),
                    ...
                    new go.Binding("strokeWidth", "highlight2", function (v) { return v ? 5 : 1; }),
                ...
                $(go.Panel, "Horizontal",
                    ...
                    $(go.Panel, "Horizontal",  // this is hidden when the swimlane is collapsed
                        ...
                        $(go.TextBlock,  // the lane label
                            {
                                font: "bold 17pt sans-serif", editable: false, margin: new go.Margin(2, 0, 0, 0), overflow: go.TextBlock.OverflowClip /* the default value */,
        ...
        myDiagram.groupTemplate.selectionAdornmentTemplate =
            $(go.Adornment, "Spot",
                ...
                $(go.Panel, "Horizontal",
                    ...
                    $("Button", { click: getResearchBasedQuestionModal }, $(go.TextBlock, "Research-Based-Question", { font: "bold 10pt sans-serif", textAlign: "center" })),
            ...
            $(go.Group, "Auto", groupStyle(),
                ...
                $(go.Panel, "Table",
                    ...
                    $(go.Panel, "Horizontal",
                        ...
                        $(go.TextBlock,
                            { font: "bold 15pt sans-serif", editable: false, margin: new go.Margin(2, 0, 0, 0) },
                            //Rank
        ...
        function showContextMenu(obj, diagram, tool) {
            ...
            maybeShowItem(document.getElementById("breakLoop"), obj !== null);
...
$(document).ready(function () {
    $('#sample').on('mousewheel', function (e) {
        if (e.originalEvent.wheelDelta / 120 > 0) {
            console.log(e.originalEvent.wheelDelta / 120);
            mouseCounter = mouseCounter + (e.originalEvent.wheelDelta / 120);
            console.log('Count', mouseCounter);
            var sD = [];
            var rank = sanPool;
            ...
            for (var i = 0; i < rank.length; i++) {
                sD.push({
                    key: rank[i].rankName, isGroup: true, category: "Pool", text: rank[i].rankName
                });
            }


            var phn = [];
            var Samphenomenon = phenomenon;
            for (var i = 0; i < Samphenomenon.length; i++) {
                phn.push({ key: Samphenomenon[i].pathwayName, text: Samphenomenon[i].pathwayName, isGroup: true, category: "Pool" });
            }

            var spc = [];
            var spc2 = [];
            var specficFinal = specfic;
            console.log("hhhhhhhhhhhhhhhhh",specficFinal);
            for (var i = 0; i < specficFinal.length; i++) {
                if (specficFinal[i].color == "#FFC6C4") {
                    spc2.push({ key: specficFinal[i].key, text: specficFinal[i].text, isGroup: true, color: specficFinal[i].color });
                }
                spc.push({ key: specficFinal[i].key, text: specficFinal[i].text, isGroup: true });
            }
            ...
            if (mouseCounter == 1) {
                ...
                init(sD, newD, isInit);
                isInit = isInit + 1;
                mainAr.push({ data: sD, data1: newD });
                forRank();
            }
            else if (mouseCounter == 2) {
                

                mainAr.push({ data: phn, data1: newD });
                isInit = isInit + 1;
                forPhenomenon();
                ...
                mainAr.push({ data: spc, data1: samData1 });
                isInit = isInit + 1;
            }
            else if (mouseCounter == 4) {
                isInit = isInit + 1;
                mainAr.push({ data: samData, data1: samData1 });

            }

            init(mainAr[mainAr.length - 1].data, mainAr[mainAr.length - 1].data1, isInit);
        }
        else {
            console.log("MINUS", e.originalEvent.wheelDelta / 120);
            mouseCounter = mouseCounter + (e.originalEvent.wheelDelta / 120);
            mainAr.splice(-1, 1)
            init(mainAr[mainAr.length - 1].data, mainAr[mainAr.length - 1].data1, isInit);
        }
...
function forRank() {
    ...
    for (var i = 0; i < rank1.length; i++) {
        sD.push({
            key: rank1[i].rankName, isGroup: true, category: "Pool", text: rank1[i].rankName
        });
    }
    $.each(sD, function (i, val) {
        rankName = rankName + "<h2>" + val.text + "</h2>";
    });
    $('#divRank').append(rankName);

}
function forPhenomenon() {
    $('#divRank').hide();
    $('#phenomenonDiv').show();
    ...
    var Samphenomenon = phenomenon;
    for (var i = 0; i < Samphenomenon.length; i++) {
        //  console.log("Samphenomenon222", Samphenomenon);
        phn.push({ key: Samphenomenon[i].pathwayName, text: Samphenomenon[i].pathwayName, isGroup: true, category: "Pool" });
    }
    $.each(phn, function (i, val) {
        phenomenonName = phenomenonName + "<h2>" + val.text + "</h2>";
    });
    $('#phenomenonDiv').append(phenomenonName);
...
function cxcommand(event, val) {
    ...
    switch (val) {
        ...
        case "breakLoop": funBreakLoop(nodeObj); break;
    }
...
function funBreakLoop(obj) {
    ...
    $.ajax({
        type: "POST",
        url: "WebService/ActivitiesApproval.asmx/funBreakLoop",
        contentType: 'application/json',
        dataType: 'json',
        data: "{'keyword': '" + nodedata.text + "','pathwayId':'" + pathwayId + "','phenomenonId':'" + nodedata.group + "','empid':'" + userLoginID + "'}",
        statusCode: {
            401: function (xhr) {
                window.location.href = "../../index.html";
            }
...
function getDetails(id) {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            //console.log(allrelation[0].linkNode);
            ...
            sanPool = pool;
            specfic = JSON.parse(allNode[0].mainNode);
            ...
            if ($("#ddlphenomenon option").length == 1) {
                ...
                console.log("table4", phenomenonList);

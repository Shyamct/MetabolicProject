        ...
        $(go.Node, "Auto",
            ...
            $(go.Panel, "Table",
                $(go.Picture,  // flag image, only visible if a nation is specified
                    { row: 0, column: 0, visible: false, desiredSize: new go.Size(16, 16) },
            ...
            $(go.Panel, "Horizontal",
                ...
                $(go.Panel, "Horizontal",  // this is hidden when the swimlane is collapsed
                    ...
                    $(go.TextBlock,  // the lane label
                        {
                            font: "bold 10pt sans-serif", editable: false, margin: new go.Margin(2, 0, 0, 0), overflow: go.TextBlock.OverflowClip /* the default value */,
    ...
    myDiagram.groupTemplate.selectionAdornmentTemplate =
        $(go.Adornment, "Spot",
            ...
            $(go.Panel, "Horizontal",
                ...
                //$("Button", { click: getMeaning }, $(go.TextBlock, "Meaning", { font: "bold 12pt sans-serif", textAlign: "left" })),
            )
        );
        ...
        $(go.Group, "Auto", groupStyle(),
            ...
            $(go.Panel, "Table",
                ...
                $(go.Panel, "Horizontal",
                    ...
                    $(go.TextBlock,
                        { font: "bold 12pt sans-serif", editable: false, margin: new go.Margin(2, 0, 0, 0) },
    ...
    myDiagram.linkTemplate =
        $(go.Link,
            ...
            $(go.Panel, "Auto",  // this whole Panel is a link label
                ...
                $(go.TextBlock, { margin: 1, font: "bold 10px Helvetica, bold Arial, sans-serif" },
...
function ShowLevel(node, linkData) {
    ...
    $.each(nodedata, function (i, val) {
        if (val.category && val.category == 'Pool') {
            rankLane.push(val);
        }
        if (Number.isInteger(val.key)) {
            receptorLane.push(val);
        }
    });

    for (var j = 0; j < receptorLane.length; j++) {

        var receptorNode = [];
        $.each(nodedata, function (i, val) {
            if (val.group == receptorLane[j].key) {
                receptorNode.push(val);
            }
        });

        var levelNodeLength = Math.ceil(receptorNode.length / level);
        var levelNode = [];
        $.each(linkdata, function (k, link) {

            if (levelNode.length < levelNodeLength) {
                $.each(receptorNode, function (i, val) {
                    if (val.key == link.from) {
                        if (!levelNode.some(data => data.key == val.key)) {
                            levelNode.push(val);
                        }
                    }
                    if (val.key == link.to) {
        ...
        $.each(levelNode, function (k, val) {
            mydata.push(val);
        });
    };

    $.each(receptorLane, function (i, val) {
        mydata.push(val);
    });

    $.each(rankLane, function (i, val) {
        mydata.push(val);
    });

    init(mydata, linkdata);
}
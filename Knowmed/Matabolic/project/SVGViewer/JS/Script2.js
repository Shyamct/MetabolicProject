        ...
        $(_svgText).bind("dblclick", function () {
            ...
            $.each(_svgText, function (i) {
                ...
                if (clickedResearchURL == researchURL) {
                    ...
                    var svgObjectSecondParent = svgObjectFirstParent.parentNode;
                    var svgObjectThirdParent = svgObjectSecondParent.parentNode;
            ...
            $.each(_svgText, function (i) {
                ...
                if (this.textContent == targetText) {
                    ...
                    if (anchorTag.nodeName == 'a') {
                        ...
                        _html += '<li><a href="' + researchURL + '" target="_blank">' + researchURL + '</a></li>';
                    }
                }

            });
            _html += '<li data-action="getMarkerDetail">Get Marker Detail</li></ul>';
...
function isValidGroup(obj) {
    if ($(obj).attr('id') && $(obj).attr('id').toLowerCase().startsWith(groupPrefix.toLowerCase())) {
        return true;
    }
    else {
        return false;
...
function filterObjectToHighlight(object) {
    ...
    _svgGroup = object.querySelectorAll(svgGroupTag);    
    ...
    if (relation && relation.length > 0) {
        ...
        $.each(_svgGroup, function (i) {
            ...
            if (isValidGroup(this)) {
                _svgText = this.querySelectorAll(svgTextTag);

                var textArray = [];
                
                if (_svgText.length > 1) {
                    var firstClass = $(_svgText[0]).attr('class');
                    var firstText = $(_svgText[0]).attr('class');
                    for (var i = 1; i < _svgText.length; i++) {
    ...
    //                    var secondStartParent = firstStartParent.parentNode;
    //                    var thirdStartParent = secondStartParent.parentNode;
    ...
    //                    var secondEndParent = firstEndParent.parentNode;
    //                    var thirdEndParent = secondEndParent.parentNode;

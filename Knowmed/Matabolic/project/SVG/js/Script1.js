...
document.getElementById('svgObject').addEventListener("load", function () {
    ...
    $(_svgObject).bind("dblclick", function () {
        ...
        if (compareString(event.target.tagName, 'text')) {
            var parent = event.target.parentNode;
            var clickedResearchURL = getAnchorURL(parent);
            while ((parent) && isEmpty(clickedResearchURL)) {
                parent = parent.parentNode;
                clickedResearchURL = getAnchorURL(parent);
            }
            if (!isEmpty(clickedResearchURL)) {
                highlightSameGroup(clickedResearchURL);
            }
        }
    });
...
function highlightSameGroup(url) {
    $.each(_svgGroup, function () {
        var g = this;
        if (isValidTextGroup(g) && $(g).has('a')) {
            var aList = g.querySelectorAll('a');
            var isMatching = false;
            $.each(aList, function () {
                var researchURL = getAnchorURL(this);
                if (!isEmpty(researchURL) && compareString(url, researchURL)) {
                    isMatching = true;
                    return false;
                }
            });
...
function getMarkerUrlList(text) {
    $.each(_svgGroup, function () {
        var g = this;
        if (isValidTextGroup(g) && compareString(extractGroupText(g), text) && $(g).has('a')) {
            var aList = g.querySelectorAll('a');
            $.each(aList, function () {
                var researchURL = getAnchorURL(this);
                if (researchURL != '' && !arrResearchURL.some(data => compareString(data.url, researchURL))) {
                    arrResearchURL.push({
                        url: researchURL
                    });
                }
...
function startHighlight(obj, section) {
    ...
    if (section) {
        if (rect) {
            ...
            else if (isStartEndPointWithRole(rect) == 'NA') {
        ...
        if (path) {
            ...
            else if (isStartEndPointWithRole(path) == 'NA') {
        ...
        if (polygon) {
            ...
            else if (isStartEndPointWithRole(polygon) == 'NA') {
        ...
        if (rect) {
            ...
            else if (checkMarkerRole(rect) == 'NA') {
                rect.classList.add("blink-Common");
            }
        }
        if (path) {
            ...
            else if (checkMarkerRole(path) == 'NA') {
                path.classList.add("blink-Common");
            }
        }
        if (polygon) {
            ...
            else if (checkMarkerRole(polygon) == 'NA') {
                polygon.classList.add("blink-Common");
            }
        }
    }
